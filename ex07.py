    ...
    def _joint_log_likelihood(self, X: np.ndarray) -> np.ndarray:
        ...
        for x in X:
            ...
            for c in self.classes_:
                ...
                row.append(
                    self.class_prior_log_[c] +
                    float(np.dot(x, self.feature_log_prob_[c]))
                )
...
def _make_gaussian_blob_data(
    ...
    mean0, cov0 = np.array([0.0, 0.0]), np.array([
        [1.0, 0.4],
        [0.4, 1.2]
    ])

    mean1, cov1 = np.array([2.5, 2.0]), np.array([
        [1.1, -0.3],
        [-0.3, 1.0]
    ])
...
if __name__ == "__main__":
    ...
    print(
        "Forward-looking note: suitable for continuous features "
    ...
    if acc_g >= 0.5 and acc_m >= 0.5:
        ...
        print(
            "Thus, the implementation executed, but model accuracy is low; "
            "consider revisiting data or smoothing."
        )